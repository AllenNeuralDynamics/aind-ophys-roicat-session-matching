...
def build_roi_table(plane, results, quality_metrics):
    ...
    roi_global_index = 0
    for session_index, session_roi_ids in enumerate(
        results["clusters"]["labels_bySession"]
    ):
        for roi_session_index, ucid in enumerate(session_roi_ids):
            ...
            sample_probabilities = (
                hdbscan["sample_probabilities"][roi_global_index] if hdbscan else None
            )
            all_rois.append(
                dict(
    ...
    roicat.helpers.save_gif(
        array=roicat.helpers.add_text_to_images(
            ...
            text=[
                ...
                for ii in range(len(ims_to_show))
            ],
            font_size=3,
            line_width=10,
            position=(30, 90),
...
def align_plane(
    ...
    assert data.check_completeness(verbose=False)[
        "tracking"
    ], f"Data object is missing attributes necessary for tracking."
    ...
    aligner = roicat.tracking.alignment.Aligner(
        verbose=True, use_match_search=True, all_to_all=all_to_all, device=DEVICE
    )
    ...
    aligner.fit_geometric(
        ...
        mask_borders=(
            ...
            50,
            50,
        ),  ## number of pixels to mask off the edges (top, bottom, left, right)
    ...
    swt = roicat.tracking.scatteringWaveletTransformer.SWT(
        kwargs_Scattering2D={
            "J": 2,
            "L": 12,
        },  ## 'J' is the number of convolutional layers. 'L' is the number of wavelet angles.
    ...
    sim.make_normalized_similarities(
        ...
        k_max=data.n_sessions
        * 100,  ## Maximum number of nearest neighbors to consider for the normalizing distribution
        k_min=data.n_sessions
        * 10,  ## Minimum number of nearest neighbors to consider for the normalizing distribution
    ...
    kwargs_makeConjunctiveDistanceMatrix_best = clusterer.find_optimal_parameters_for_pruning(
        ...
        bounds_findParameters={
            "power_NN": (0.0, 2.0),  ## Bounds for the exponent applied to s_NN
            "power_SWT": (0.0, 2.0),  ## Bounds for the exponent applied to s_SWT
            "p_norm": (
                -5,
                -0.1,
            ),  ## Bounds for the p-norm p value (Minkowski) applied to mix the matrices
            "sig_NN_kwargs_mu": (0.0, 1.0),  ## Bounds for the sigmoid center for s_NN
            "sig_NN_kwargs_b": (0.1, 1.5),  ## Bounds for the sigmoid slope for s_NN
            "sig_SWT_kwargs_mu": (0.0, 1.0),  ## Bounds for the sigmoid center for s_SWT
    ...
    (
        ...
        labels_bySession,
        labels_bool,
        labels_bool_bySession,
        labels_dict,
    ) = roicat.tracking.clustering.make_label_variants(
        labels=labels, n_roi_bySession=data.n_roi
    )
    ...
    results_all = {
        "clusters": {
            ...
            "quality_metrics": (
                roicat.util.JSON_Dict(clusterer.quality_metrics)
    ...
    logging.info(
        f'Number of discarded ROIs: {(np.array(results_clusters["labels"])==-1).sum()}'
    )

    FOV_clusters = roicat.visualization.compute_colored_FOV(
        spatialFootprints=[
            r.power(1.0) for r in results_all["ROIs"]["ROIs_aligned"]
        ],  ## Spatial footprint sparse arrays
    ...
    roicat.helpers.json_save(
        obj=results_clusters, filepath=paths_save["results_clusters"]
    )
    roicat.helpers.json_save(obj=params_used, filepath=paths_save["params_used"])
    roicat.util.RichFile_ROICaT(path=paths_save["results_all"]).save(
        obj=results_all, overwrite=True
    )
    roicat.util.RichFile_ROICaT(path=paths_save["run_data"]).save(
        obj=run_data, overwrite=True
    )
    ...
    roicat.helpers.save_gif(
        array=roicat.helpers.add_text_to_images(
            ...
            text=[
                ...
                for ii in range(len(FOV_clusters))
            ],
            font_size=3,
            line_width=10,
            position=(30, 90),
