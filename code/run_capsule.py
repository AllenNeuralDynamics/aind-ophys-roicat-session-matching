...
def run():
    ...
    args.nonrigid_method = (
        None if args.nonrigid_method == "Off" else args.nonrigid_method
    )
    args.all_to_all = args.all_to_all == "on"

    logging.info(
        f"geometric method: {args.geometric_method}, {GEOMETRIC_METHOD_DEFAULTS.get(args.geometric_method)}"
    )
    logging.info(
        f"nonrigid method: {args.nonrigid_method}, {NONRIGID_METHOD_DEFAULTS.get(args.nonrigid_method)}"
    )
    ...
    for name, plane in planes.items():
        ...
        outputs.append(
            dict(t_start=t_start, t_end=t_end, results=results, plane_name=name)
        )
